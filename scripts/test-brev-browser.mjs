...
const kall=[];
const server=lagServer({katalog:path.join(dir,"data"),nett:{hentSide:async url=>({html:`<main>${annonse}</main>`,sluttUrl:url})},brevModellFor:()=>async ({trinn,innhold,signal})=>{
  kall.push(trinn);
  await new Promise(r=>setTimeout(r,450));signal?.throwIfAborted();
...
try{
  ...
  page=await context.newPage();const feil=[];
